...
for lang in os.listdir(locale):
    ...
    for root, _, files in os.walk(f"{locale}/{lang}"):
        ...
        for f in files:
            ...
            with open(f"{root}/{f}") as fi:
                strings = sorted([l.replace("{{", "{").replace("}}", "}")
    ...
    if intents:
        with open(f"{tx}/{lang.lower()}/intents.json", "w") as f:
            json.dump(intents, f, indent=4, ensure_ascii=False, sort_keys=True)
    if dialogs:
        with open(f"{tx}/{lang.lower()}/dialogs.json", "w") as f:
            json.dump(dialogs, f, indent=4, ensure_ascii=False, sort_keys=True)
    if vocs:
        with open(f"{tx}/{lang.lower()}/vocabs.json", "w") as f:
            json.dump(vocs, f, indent=4, ensure_ascii=False, sort_keys=True)
    if regexes:
        with open(f"{tx}/{lang.lower()}/regexes.json", "w") as f:
            json.dump(regexes, f, indent=4, ensure_ascii=False, sort_keys=True)
